...
$processes = @(
    Get-CimInstance Win32_Process |
        Where-Object {
            ...
            $_.CommandLine -match "run_pytest_shard\.py|run_gate_shard\.py|(^|\s)-m\s+pytest(\s|$)|pytest(\.exe)?(\s|$)"
        } |
        Sort-Object ProcessId |
        Select-Object ProcessId, ParentProcessId, Name, CreationDate, CommandLine
)
$payload = [ordered]@{
    schema_version = "dawnstrike.final_commit_gate_process_audit.v1"
    captured_at_utc = [DateTimeOffset]::UtcNow.ToString("o")
    unauthorized_process_count = $processes.Count
    processes = $processes
}
$rendered = $payload | ConvertTo-Json -Depth 6
