...
| Vercel packaging | `BLOCKED_EXTERNAL` | Minimal static stage and liveness/readiness contracts are implemented; the current real-data artifact is correctly rejected as dirty/degraded, and local CLI packaging requires missing `uv`. |
