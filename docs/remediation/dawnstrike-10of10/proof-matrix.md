...
|---|---|---|
| Full pytest suite | PASS: 217 passed | `py -m pytest -q` |
...
| Diagnostic public build | PASS/DEGRADED | 425 canonical rows, 222 daily records, 607,268 raw bytes / 37,457 deterministic-gzip bytes, 250 public rows, snapshot `degraded`, readiness HTTP 503 |
| Artifact verifier | FAIL-CLOSED | Build source SHA `6e172db6d32844d93f63057ac28df50f052ce1d6` rejects only `snapshot_not_publishable` and `readiness_not_publishable`; compressed-size and row-limit checks pass |
...
| Vercel-native build | PASS/NO_DEPLOY | Explicit Dawnstrike project build succeeds with Vercel CLI 58.4.0; 18 prebuilt files, 834,208 bytes, two functions, zero forbidden files |
