...
def evaluate_policy(
    ...
    *,
    ...
    missing = [spec for key, spec in specs.items() if key not in by_id]
    ...
    for spec in specs.values():
        ...
        if result is None:
            if spec.blocking_for_research_pick:
                all_failures.append(spec.condition_id)
                (
                    safety_failures
                ...
                disclosed.append(spec.condition_id)
            continue
        if (
            ...
            all_failures.append(spec.condition_id)
            data_failures.append(spec.condition_id)
            continue
        if result.status in _BLOCKING_BAD:
            if spec.blocking_for_research_pick:
                all_failures.append(spec.condition_id)
                if spec.category == ConditionCategory.HARD_RISK:
                    safety_failures.append(spec.condition_id)
                else:
                    data_failures.append(spec.condition_id)
                ...
                disclosed.append(spec.condition_id)
        elif result.status == ConditionStatus.MISSING_DISCLOSED:
            disclosed.append(spec.condition_id)
    del missing
    if reward_risk_ratio is None or reward_risk_ratio < 1.50:
        all_failures.append("reward_risk_at_least_1_50")
        data_failures.append("reward_risk_at_least_1_50")
    if base_score < score_threshold:
        all_failures.append("deterministic_score_threshold")
        data_failures.append("deterministic_score_threshold")
    ...
    if strategy_id == "cross_sectional_relative_strength":
        ...
        if sector is None or sector.status in {
            ...
            disclosed.append("sector_industry:UNKNOWN")
