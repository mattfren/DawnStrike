...
def build_alpha_run_contract(
    *,
    ...
    elif _all_plan_inputs_ineligible(signals) or (
        int(enrichment.get("selected_count") or 0) > 0
        and int(enrichment.get("verified_count") or 0) == 0
    ):
    ...
    return AlphaRunContract(
        ...
        primary_veto=str(
            decision.get("primary_reason_code")
