...
def classify_canonical_data(
    *,
    ...
    warnings = tuple(
        dict.fromkeys(manifest.warnings + reconciliation.warnings)
    )
    ...
    if status in {"reconciled", "reconciled_with_minor_diffs"}:
        return CanonicalDataDecision(
            ...
            allow_forward=True,
            reason=(
                "provider data is reconciled within configured tolerances"
            ),
            warnings=warnings,
        )
    if status == "single_provider_unreconciled":
        return CanonicalDataDecision(
            ...
            warnings=warnings
            + ("single-provider data is not independently reconciled",),
        )
    if status == "mismatch":
        return CanonicalDataDecision(
            canonical_snapshot_id=manifest.snapshot_id,
