...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "command",
        choices=("build", "import-csv", "summary"),
    )
    ...
    if args.command == "import-csv":
        ...
        print(
            f"Imported snapshot: "
            f"{import_result.snapshot.manifest.snapshot_id}"
        )
        print(
            f"Accepted bars: "
            f"{import_result.snapshot.manifest.accepted_bar_count}"
        )
        print(f"Rejected bars: {import_result.rejected_bar_count}")
        print(
            f"Skipped incomplete bars: "
            f"{import_result.skipped_incomplete_bars}"
        )
        try:
            canonical_dataset, canonical_manifest = (
                load_datatruth_dataset(output_root=output_root)
            )
            ...
            comparison_datasets = {
                import_result.snapshot.provider_id: (
                    import_result.snapshot.dataset
                )
            }
        ...
        artifacts = write_reconciliation_v2(
...
def _write_import_summary(
    ...
    lines = [
        ...
        (
            f"- Skipped incomplete bars: "
            f"`{import_payload['skipped_incomplete_bars']}`"
        ),
